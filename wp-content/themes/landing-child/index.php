						...
						<div class="item-banner relative z-1">
							...
							<div class="md:text-left text-center mx-auto px-3 container top-[50%] left-[50%] md:-translate-y-[50%] -translate-y-[80%] -translate-x-[50%] absolute z-1">
								<div class="box-banner flex max-w-[40.125rem] mr-auto">
									<div class="box-top">
										<p class="roboc font-bold uppercase lg:text-[2.5rem] md:text-[1.5rem] text-[1.35rem] text-white lg:mb-[2.1875rem] md:mb-[1.25rem] mb-[1rem] leading-snug">
											<?php echo _cget('name_banner', $banner); ?>
										</p>
										<div class="short text-base text-white">
											<?php echo _cget('desc_banner', $banner); ?>
										</div>
										<a href="<?php echo _cget('banner_link', $banner); ?>" title="Xem ngay" class="inline-block lg:text-xl md:text-base text-sm xl:mt-[7.3125rem] lg:mt-[5.3125rem] mt-[2.3125rem] bg-yellow p-3 rounded rounded-[0.625rem] px-24">Xem ngay</a>
									</div>
								</div>
							</div>
						</div>
...
<?php if (!empty($steps)) : ?>
	<section class="section-start-build mb-3">
		<div class="banner mb-3 wow fadeInUp">
			<img src="<?php echo mb_image("start_img_desktop"); ?>" class="img-full" alt="Bạn không biết bắt đầu như thế nào ?" />
		</div>
			...
			<div class="flex flex-wrap gap-[0.2688rem]">
				<div class="col md:flex-[0_0_59%] flex-[0_0_100%] flex md:gap-3 gap-2 flex-wrap">
					...
					<?php foreach ($steps as $k => $step) : ?>
						...
						<div class="wow fadeInUp item md:p-[1.25rem] md:flex-[0_0_48.50%] flex-[0_0_100%] p-[0.75rem] md:bg-[#FDF2B2] bg-blue hover:bg-[#FDD501]" data-content="<?php echo _cget('step_full_desc', $step); ?>" data-content="<?php echo $imgFull ?? ''; ?>">
							...
							<?php if (!empty($imgsIcon)) : ?>
								...
								<?php endforeach; ?>
							<?php endif; ?>
							<div class="short lg:text-[1.5rem] text-[1.25rem] md:text-blue text-white md:min-h-[7.25rem]">
				...
				<div class="col md:flex-[0_0_39%] flex-[0_0_100%]">
					<div class="item-large pt-[2rem] px-[1.225rem] pb-[1rem] bg-yellow h-full wow fadeInUp">
						<div class="short g:text-[1.5rem] text-[1.25rem] lg:mb-5 mb-3">
							<?php echo _cget('step_full_desc', $steps[0]); ?>
						</div>
						<?php $imgsIcon = _cget('step_img_desktop', $steps[0], []); ?>
						<?php if (!empty($imgsIcon)) : ?>
							<?php foreach ($imgsIcon as $k2 => $img) : ?>
								<img src="<?php echo wp_get_attachment_image_url($img, 'full') ?>" alt="<?php echo _cget('name_step', $steps[0]); ?>" class="md:w-[32.5rem] md:h-[32.5rem] block mx-auto" />
							<?php endforeach; ?>
						<?php endif; ?>
					</div>
				</div>
...
<?php if (!empty($whys)) : ?>
	<section class="section-why bg-[#f2f2e8] pb-[3.75rem]">
		<div class="banner wow fadeInUp">
			<img src="<?php echo mb_image("why_img_desktop"); ?>" class="img-full" alt="Tại sao chọn chúng tôi" />
		</div>
		<div class="container mx-auto px-3">
			<div class="flex flex-wrap lg:gap-3 gap-2 lg:mb-6 mb-4 navv">
				<?php foreach ($whys as $k => $why) : ?>
					<div class="col md:flex-[0_0_24%] flex-[0_0_100%] wow fadeInUp nav-item" data-state="<?php echo $k == 0 ? 'true' : 'false' ?>" id="step-tab-<?php echo $k; ?>" data-target="#step-<?php echo $k; ?>">
						<div class="roboc item-step text-center font-bold p-[0.5rem] bg-blue hover:bg-yellow text-yellow hover:text-blue text-yellow relative z-5">
			...
			<div class="tabs bg-[#FAD52C] panel">
				<?php foreach ($whys as $k => $why) : ?>
					<div data-state="<?php echo $k == 0 ? 'true' : 'false' ?>" id="step-<?php echo $k; ?>" class="tab-panel flex flex-wrap relative z-1 item-tab lg:p-[3.4375rem] md:p-[2.4375rem] p-[1.4375rem] justify-between <?php echo $k == 0 ? 'flex' : 'hidden' ?>">
						<div class="col md:flex-[0_0_60%] flex-[0_0_100%]">
							<div class="image">
								<?php $imgsFull = _cget('why_img_desktop', $why, []); ?>
								<?php if (!empty($imgsFull)) : ?>
									<?php foreach ($imgsFull as $k2 => $img) : ?>
										<img src="<?php echo wp_get_attachment_image_url($img, 'full') ?>" alt="<?php echo _cget('name_why', $why); ?>" class="block mx-auto max-w-[41.875rem] max-h-[41.875rem]" />
									<?php endforeach; ?>
								<?php endif; ?>
							</div>
						</div>
						...
						<?php if ($benefits != "") : ?>
							<div class="col md:flex-[0_0_37.5%] flex-[0_0_100%] flex flex-wrap items-center md:mt-0 mt-3">
								<?php foreach ($benefits as $k1 => $benefit) : ?>
									<div class="md:block flex items-center item_why mb-3">
										...
										<?php if (!empty($imgsIcon)) : ?>
											...
											<?php endforeach; ?>
										<?php endif; ?>
										<p class="lg:text-[1.125rem] md:text-base text-xs md:mt-3 mt-0 md:pl-0 pl-3">
...
<?php if (!empty($teachers)) : ?>
	<section class="container px-3 mx-auto section-teacher xl:mt-[3.125rem] lg:mt-[2.25rem] mt-[1.5rem] pb-[2.5rem]">
		<p class="title roboc lg:text-[4rem] md:text-[2.5rem] text-[1.5rem] font-bold text-center uppercase text-blue wow fadeInUp">
			<?php echo mb_option("title_teacher_1"); ?>
		</p>
		<div class="short_title roboc lg:text-[4rem] md:text-[2.5rem] text-[1.5rem] font-bold text-center uppercase text-[#FDD501] mb-3 wow fadeInUp">
		...
		<div class="flex flex-wrap gap-2 items-center">
			<div class="col left lg:flex-[0_0_39%] lg:max-w-[39%] flex-[0_0_100%]">
				<div class="swiper-teacher-main swiper">
					<div class="swiper-wrapper">
						<?php foreach ($teachers as $k => $teacher) : ?>
							<div class="swiper-slide">
								<div class="item-teacher text-[#FDD501] bg-yellow px-[1.25rem] py-[2rem] wow fadeInUp">
			...
			<div class="col right lg:flex-[0_0_59%] flex-[0_0_100%] lg:max-w-[59%]">
				<div class="swiper-teacher-thumb swiper">
					<div class="swiper-wrapper">
						<?php foreach ($teachers as $k => $teacher) : ?>
							<div class="swiper-slide mb-2 wow fadeInUp">
								<div class="teacher mr-2">
									<?php $imgs = _cget('teacher_icon_desktop', $teacher, []); ?>
									<?php if (!empty($imgs)) : ?>
										<?php foreach ($imgs as $k2 => $img) : ?>
											<img src="<?php echo wp_get_attachment_image_url($img, 'full') ?>" alt="<?php echo _cget('name_teacher', $teacher); ?>" class="img-full" />
										<?php endforeach; ?>
									<?php endif; ?>
								</div>
							</div>
...
<?php if (!empty($videos)) : ?>
	<section class="section-video bg-[#FFFDF1] lg:py-[2.5rem] py-[1.75rem] relative z-10">
		<div class="banner mb-3 wow fadeInUp">
			<img src="<?php echo mb_image("banner_video"); ?>" class="img-full" alt="Video" />
		</div>
		<div class="container mx-auto px-3">
			<div class="swiper swiper-video">
				<div class="swiper-wrapper">
					<?php foreach ($videos as $k => $video) : ?>
						<div class="swiper-slide mb-4">
							<div class="item-video mr-4 wow fadeInUp">
								<a href="<?php echo _cget('link_video', $video); ?>" title="<?php echo _cget('name_video', $video); ?>" class="block play-video" data-href="<?php echo _cget('link_video', $video); ?>">
									<?php $imgs = _cget('img_video', $video, []); ?>
									<?php if (!empty($imgs)) : ?>
										<?php foreach ($imgs as $k2 => $img) : ?>
											<img src="<?php echo wp_get_attachment_image_url($img, 'full') ?>" alt="<?php echo _cget('name_video', $video); ?>" class="img-full" />
										<?php endforeach; ?>
									<?php endif; ?>
								</a>
							</div>
				...
				<div class="swiper-control flex items-center justify-center absolute top-[50%] -translate-y-[1/2] swiper-prev z-1 left-0">
					<i class="fa fa-angle-left" aria-hidden="true"></i>
				</div>
				<div class="swiper-pagi static blue"></div>
				<div class="swiper-control flex items-center justify-center absolute top-[50%] -translate-y-[1/2] swiper-next z-1 right-0">
	...
	<div class="container mx-auto px-3">
		<div class="grid grid-cols-2 mx-auto">
			<div class="md:col-span-1 col-span-2 md:mb-0 mb-3 max-w-[31.25rem]">
				<p class="md:text-[1.25rem] text-base text-white mb-1 font-bold roboc md:text-left text-center wow fadeInUp">
					<?php echo mb_option("title_sale_1"); ?>
				</p>
				<p class="md:text-[1.25rem] text-base text-yellow mb-2 font-bold roboc md:text-left text-center wow fadeInUp">
					<?php echo mb_option("title_sale_2"); ?>
				</p>
				<div class="desc text-white lg:text-left text-center wow fadeInUp">
					<?php echo mb_option("desc_sale"); ?>
				</div>
			</div>
			<div class="md:col-span-1 col-span-2 max-w-[29.6875rem] md:ml-auto">
				<div class="text-sale md:text-base text-[1.5rem] text-white md:text-right text-center leading-snug relative z-1 wow fadeInUp">
					Học phí chỉ từ
					<span class="wow fadeInUp lg:text-[3.275rem] md:text-[1.5rem] text-[3.25rem] font-bold text-[#fad15e] roboc md:inline-block block"><?php echo mb_option("price_sale"); ?></span>
					<span class="wow fadeInUp md:text-base text-sm sm:absolute lg:static top-[2rem] right-[1.5rem] z-1">đ/tháng</span>
				</div>
				<a href="<?php echo site_url(); ?>/#contact-now" title="NHẬN TƯ VẤN LỘ TRÌNH HỌC" class="wow fadeInUp block w-fit p-[0.5688rem] px-5 rounded rounded-[1.875rem] text-white bg-[#ffac00] shadow-lg mx-auto mt-4 font-bold roboc">
...
<?php if (!empty($faqs)) : ?>
	<section class="section-faq lg:py-[4.375rem] md:py-[3.375rem] py-[2.375rem]">
		<div class="container mx-auto px-3 max-w-[40.3125rem]">
			<p class="title lg:text-[1.5rem] text-[1.25rem] bg-[#FFD602] text-blue text-center p-2 block mb-3 font-bold wow fadeInUp">
				<?php echo mb_option("title_faq"); ?>
			</p>
			<div class="ls-faq">
				<?php foreach ($faqs as $k => $faq) : ?>
					<div class="item-faq bg-[#FFFBE5] hover:bg-yellow mb-2 wow fadeInUp">
						<p class="text-center py-[0.7688rem] px-3">
							<?php echo _cget('name_question', $faq); ?>
						</p>
						<div class="content py-[0.4688rem] px-3 s-content" style="display: none;">
	...
	<div class="container mx-auto px-3">
		<div class="grid grid-cols-2 lg:gap-4 md:gap-3 gap-2">
			<div class="md:col-span-1 col-span-2 wow fadeInUp">
				<a href="tel:<?php echo mb_option("hotline"); ?>" title="<?php echo mb_option("hotline"); ?>" class="w-fit block mx-auto text-blue mb-3 text-center font-bold text-base">Hotline: <?php echo mb_option("hotline"); ?></a>
			...
			<div class="md:col-span-1 col-span-2 wow fadeInUp">
				<p class="w-fit block mx-auto text-blue mb-3 text-center font-bold text-base uppercase">
						...
						<div class="swiper swiper-partner wow fadeInUp">
							<div class="swiper-wrapper">
								<?php foreach ($partners as $k => $partner) : ?>
									<div class="swiper-slide">
										<?php $imgsDesktop = _cget('banner_partner_desktop', $partner, []); ?>
										<?php if (!empty($imgsDesktop)) : ?>
											<?php foreach ($imgsDesktop as $k2 => $img) : ?>
