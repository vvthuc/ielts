...
add_action( 'admin_init', function() {
	if (class_exists('MB_ADMIN_COLUMNS_POST')) {
    	class ADMIN_COLUMNS_PRODUCT_MBAC extends MB_ADMIN_COLUMNS_POST { 
    	}
	} elseif (class_exists('\MBAC\Post')) {
    	class ADMIN_COLUMNS_PRODUCT_MBAC extends \MBAC\Post {
    	}
	}
	class Product_Custom_Admin extends ADMIN_COLUMNS_PRODUCT_MBAC {
		public function columns( $columns ) {
			$columns  = parent::columns( $columns );
			$position = 'after';
			$target   = 'title';
			$this->add( $columns, 'home', 'Show home', $position, $target );
	...
	new Product_Custom_Admin( 'product', array() );
